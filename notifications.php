...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_notification'])) {
    ...
    $stmt = $pdo->prepare("
        ...
        VALUES (?, ?, NOW())
    ");
    $stmt->execute([$resident_id, $description]);

    $log->info('Отправлено уведомление', ['resident_id' => $resident_id, 'role' => $roleName]);
...
$notifications = $pdo->query("
    SELECT n.*, r.last_name, r.first_name, r.inidroom 
    FROM notifications n
    JOIN residents r ON n.id_residents = r.id
    ORDER BY n.create_date DESC
")->fetchAll();

// Все жители для формы
$residents = $pdo->query("SELECT * FROM residents ORDER BY last_name, first_name")->fetchAll();
?>
        ...
        <table style="width: 100%; border-collapse: collapse; table-layout: fixed;">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="padding:12px;"><?= htmlspecialchars($n['last_name'] . ' ' . $n['first_name']) ?> (<?= $n['inidroom'] ?>)</td>
