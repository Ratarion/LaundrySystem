...
from sqlalchemy import join, select, update, delete, and_, func, extract, Integer, or_, cast, Date, case
...
async def find_resident_by_id_card(id_card: str):
...
async def get_all_users_with_vk(dormitory_id: Optional[int] = None) -> List[tuple[int, str]]:
    """
    Возвращает список кортежей (vk_id, language) всех пользователей, привязанных к VK, у которых включены уведомления.
    """
    async with async_session() as session:
        query = select(User.vk_id, User.language).where(
...
async def get_user_notify_status_by_vk(vk_id: int) -> bool:
    """Возвращает статус подписки на уведомления о свободных слотах для пользователя VK."""
    async with async_session() as session:
        query = select(User.notify_unconfirmed).where(User.vk_id == vk_id)
        result = await session.execute(query)
        val = result.scalar_one_or_none()
        return True if val is None else bool(val)


async def toggle_user_notify_by_vk(vk_id: int) -> bool:
...
async def get_bookings_to_remind_vk(minutes_before: int = 60, minutes_deadline: int = 30):
    """Ищет записи, до начала которых от minutes_deadline до minutes_before минут, и которым еще не отправлялось напоминание в VK."""
    now = get_kemerovo_now()
    ...
    async with async_session() as session:
        query = select(Booking).options(joinedload(Booking.user), joinedload(Booking.machine)).where(
            and_(
                ...
                or_(Booking.reminded_vk == False, Booking.reminded_vk == None)
            )
        )
        result = await session.execute(query)
        return result.scalars().all()

async def mark_booking_reminded_vk(booking_id: int):
    """Отмечает, что напоминание в VK отправлено, и переводит статус в 'Ожидание подтверждения'."""
    async with async_session() as session:
...
async def autocancel_booking(booking_id: int, platform: str = "vk"):
...
async def get_autocanceled_to_notify_vk():
    """Ищет автоотмененные записи, по которым еще не отправлено уведомление в VK."""
    now = get_kemerovo_now()
    async with async_session() as session:
        query = select(Booking).options(joinedload(Booking.machine), joinedload(Booking.user)).where(
            and_(
                Booking.is_autocanceled == True,
                or_(Booking.canceled_notified_vk == False, Booking.canceled_notified_vk == None),
...
async def mark_autocanceled_notified_vk(booking_id: int):
    """Отмечает, что уведомление об автоотмене отправлено в VK."""
    async with async_session() as session:
        query = update(Booking).where(Booking.id == booking_id).values(canceled_notified_vk=True)
        await session.execute(query)
        await session.commit()
...
async def get_bookings_to_remind(minutes_before: int = 60, minutes_deadline: int = 30):
    return await get_bookings_to_remind_vk(minutes_before, minutes_deadline)
